...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	public final T getRefreshableView() {
		// 消除上下拉是的蓝色
		refreshableView.setOverScrollMode(View.OVER_SCROLL_NEVER);
		return refreshableView;
	}
	...
	protected void addRefreshableView(Context context, T refreshableView) {
		addView(refreshableView,
				new LinearLayout.LayoutParams(android.view.ViewGroup.LayoutParams.FILL_PARENT, 0, 1.0f));
	}
	...
	protected final void smoothScrollTo(int y) {
		...
		if (this.getScrollY() != y) {
			Log.e("tonglu", "getScrollY():" + this.getScrollY());
			Log.e("tonglu", "getScrollY():" + y);
			this.currentSmoothScrollRunnable = new SmoothScrollRunnable(handler, getScrollY(), y);
			handler.post(currentSmoothScrollRunnable);// 在线程中启动一个新的线程
		}
	}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		touchSlop = ViewConfiguration.getTouchSlop();// 触发移动事件的最短距离

		// Styleables from XML
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.LibPullToRefresh);
		if (a.hasValue(R.styleable.LibPullToRefresh_mode)) {
			mode = a.getInteger(R.styleable.LibPullToRefresh_mode, MODE_PULL_DOWN_TO_REFRESH);
		}
		...
		// Styleables from XML
		if (a.hasValue(R.styleable.LibPullToRefresh_headerTextColor)) {
			final int color = a.getColor(R.styleable.LibPullToRefresh_headerTextColor, Color.BLACK);
			if (null != headerLayout) {
		...
		if (a.hasValue(R.styleable.LibPullToRefresh_headerBackground)) {
			this.setBackgroundResource(a.getResourceId(R.styleable.LibPullToRefresh_headerBackground, Color.WHITE));
		}
		if (a.hasValue(R.styleable.LibPullToRefresh_adapterViewBackground)) {
			refreshableView.setBackgroundResource(a.getResourceId(R.styleable.LibPullToRefresh_adapterViewBackground,
					Color.WHITE));
		}
		a.recycle();
	...
	private void measureView(View child) {
		...
		// 计算一个合适子视图的尺寸大小（宽度或者高度)
		int childWidthSpec = ViewGroup.getChildMeasureSpec(0, 0 + 0, p.width);
		int lpHeight = p.height;
		int childHeightSpec;
		if (lpHeight > 0) {
			// 根据提供的大小值和模式创建一个测量值(格式)
			childHeightSpec = MeasureSpec.makeMeasureSpec(lpHeight, MeasureSpec.EXACTLY);// EXACTLY(完全)父元素决定子元素的确切大小，子元素将被限定在给定的边界里而忽略它本身大小
		} else {
			childHeightSpec = MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED);// UNSPECIFIED(未指定),父元素不对子元素施加任何束缚，子元素可以得到任意想要的大小
		}
