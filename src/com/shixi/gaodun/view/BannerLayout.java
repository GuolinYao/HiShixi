 ...
 * 
 */
public class BannerLayout extends AdapterView<Adapter> {
	...
	private boolean isFinishSlide;// 是否停止滑动
	...
	public BannerLayout(Context context, int sideBuffer) {
		super(context);
	...
	public BannerLayout(Context context, AttributeSet attrs) {
		...
		TypedArray styledAttrs = context.obtainStyledAttributes(attrs,
				R.styleable.BannerLayout);
		mSideBuffer = styledAttrs
				.getInt(R.styleable.BannerLayout_sidebuffer, 3);
		init();
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			// 处理事件，左右滑动时，传递给子项处理，上下滑动时交由viewgroup处理
			if (null != viewGroup) {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			// // 如果down与up距离很小，认为是点击事件 TODO
			// float newX = ev.getX();
			// float dx = Math.abs(newX - mDownX);
			//
			// if (dx < 200) {
	...
	private void snapToScreen(int whichScreen) {
		...
		//100 ：设置滑动时间
		mScroller.startScroll(getScrollX(), 0, delta, 0, 100);
//		mScroller.startScroll(getScrollX(), 0, delta, 0, Math.abs(delta) * 2);
	...
	public void setSelection(int position) {
		...
		mCurrentAdapterIndex = position;// 设置开始位置
